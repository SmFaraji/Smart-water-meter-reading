...
noOfclsses = len(myList)
print("Important Classes..........")

for x in range(0, noOfclsses):
...
plt.bar(range(0, noOfclsses), numOfSamples)
...
dataGen.fit(x_train)
y_train = to_categorical(y_train, noOfclsses)
y_test = to_categorical(y_test, noOfclsses)
y_validation = to_categorical(y_validation, noOfclsses)
...
def myModel():
    ...
    model.add(Dropout(0.5))
    model.add(Dense(noOfclsses, activation='softmax'))
...
print('saving.....')
# joblib.dump(model, 'model_trained3.sav')
model.save('A/DigitDetector_50epochs.h5') # , save_format="h5"
# pickle_out = open("A/model_trained2.p", "wb")
# pickle.dump(model, pickle_out)
# pickle_out.close()
# tf.keras.models.save_model(model, "A/model_trained1")
print('saved.')
